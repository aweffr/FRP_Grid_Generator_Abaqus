...
# 起吊点高度
time = f["time"]
caeName = 'trueMpdel-Closer-%d'%time
jobName = 'trueModel-Closer-%d'%time
leftHeight = 10.0
rightHeight = 12.0
leftHangPoint = 18.0
rightHangPoint = 42.0
...
set_loadLeft = myAssembly.Set(
    name='Set-loadLeft', 
    vertices=Instance_B.vertices.getByBoundingBox(xMin=leftHeight-0.1, xMax=leftHangPoint+0.1, yMin=-0.1, yMax=0.1, zMin=-0.1, zMax=0.1)
    )
set_loadRight = myAssembly.Set(
    name='Set-loadRight', 
    vertices=Instance_B.vertices.getByBoundingBox(xMin=rightHangPoint-0.1, xMax=rightHangPoint+0.1, yMin=-0.1, yMax=0.1, zMin=-0.1, zMax=0.1)
    )
...
submitJob = mdb.Job(atTime=None, 
        ...
        numCpus=8, numDomains=8, numGPUs=900, queue=None, 
        scratch='', 
        type=ANALYSIS,
        userSubroutine='', 
